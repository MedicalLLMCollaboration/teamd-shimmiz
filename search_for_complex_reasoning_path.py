...
def parse_gpt_response(response):
    try:
        ...
        assert da["CoT"][-3]["action"] == "Inner Thinking", (
            "Inner Thinking should be the third last action"
        )
        assert da["CoT"][-2]["action"] == "Final Conclusion", (
            "Final Conclusion should be the second last action"
        )
        assert da["CoT"][-1]["action"] == "Verification", (
            "Verification should be the last action"
        )
        return True, da
...
def get_stream_of_search(longcot):
    ...
    for x in longcot:
        if "title" in x:
            ...
            resstr.append(
                temp.format(
                    x["action"].replace("Final Conclusion", "Conclusion"), x["content"]
                )
            )
    return "\n".join(resstr).strip()
...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, required=True, help="Path to the input JSON data file."
    )
    parser.add_argument(
        "--model_name", type=str, default="gpt-4", help="Name of the GPT model to use."
    )
    ...
    parser.add_argument(
        "--max_search_depth", type=int, default=2, help="Maximum search depth."
    )
    parser.add_argument(
        "--efficient_search",
        type=bool,
        default=True,
        help="Enable efficient search strategy.",
    )
    parser.add_argument(
        "--num_process", type=int, default=5, help="Number of parallel processes."
    )
    parser.add_argument(
        "--limit_num", type=int, help="Limit the number of processed items."
    )
    ...
    def filter_data(tmpdata):
        ...
        for da in tmpdata:
            if (
                "Open-ended Verifiable Question" not in da
                or "Ground-True Answer" not in da
            ):
                continue
            filtered_data.append(da)

        print(
            f"Original data size: {len(tmpdata)}, Filtered data size: {len(filtered_data)}"
        )
    ...
    gpt_instance = GPT(
        model_name=args.model_name, api_url=args.api_url, api_key=args.api_key
    )
    ...
    def write_piece_order_data(d):
        ...
        try:
            ...
            for rethinking_try_time in range(args.max_search_attempts):
                ...
                for rethink_time in range(args.max_search_depth):
                    ...
                    reasoning = json.dumps(
                        d["Long_CoT"][:-1], ensure_ascii=False, indent=2
                    )
                    # Search strategy
                    if rethink_time > 0:
                    ...
                    query = strategy.format(
                        d["Open-ended Verifiable Question"], reasoning
                    )
                    d["gpt4_query_cot"].append(query)
            ...
            if not d["verify"][-1] and args.efficient_search:
                ...
                query = gen_prompt_w_label.format(
            ...
            if d["verify"][-1]:
                ...
                query = reformat_to_complex_cot_prompt.format(
                    sos, d["Open-ended Verifiable Question"]
                )
                d["gpt4_query_cot"].append(query)
                for ii in range(retry_time):
                    response = gpt_instance.retry_call(query)
                    ...
                    if flag:
                        ...
                        # get response
                        query = get_final_response_prompt.format(
                            d["Complex_CoT"], d["Open-ended Verifiable Question"]
                        )
                        d["gpt4_query_cot"].append(query)
                        response = gpt_instance.retry_call(query)
                        d["gpt4_response_cot"].append(response)
    ...
    with ThreadPoolExecutor(max_workers=args.num_process) as executor:
        list(
            tqdm(
                executor.map(write_piece_order_data, data),
                total=len(data),
