...
def parse_arguments():
    ...
    parser.add_argument(
        "--filter_data",
        action="store_true",
        help="Enable filtering of questions with LLMs.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_process", type=int, default=10, help="Number of parallel processes."
    )
    parser.add_argument(
...
def parse_gpt_response(response):
    try:
        ...
        assert isinstance(parsed_data["Open-ended Verifiable Question"], str), (
            "Open-ended Question must be a string."
        )
        assert isinstance(parsed_data["Ground-True Answer"], str), (
            "Ground-True Answer must be a string."
        )
...
def process_single_item(
    item, gpt_instance, save_directory, filter_prompt, reformat_prompt, filter_enabled
):
    try:
        max_retries = 2
        ...
        for _ in range(max_retries):
            ...
            if valid:
                item["Open-ended Verifiable Question"] = parsed_data[
