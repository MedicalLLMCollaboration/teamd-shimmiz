...
Your task is to evaluate the model response by comparing it to the reference answer. If the model response is correct and aligns with the reference answer, output "True" . If it is incorrect or fails to select the correct option (if options are provided), output "False" . {}"""
    ...
    for i in range(len(sub_answer)):
        ...
        format_response = tmp.format(
            response, sub_answer[i], reward_tokenizer.eos_token
        )
        processed_batch.append(format_response)

    input_batch = reward_tokenizer(
        processed_batch,
        return_tensors="pt",
    ...
    with torch.no_grad():
        ...
        rewards = torch.tensor(
            rewards_list, device=probabilities.device, dtype=probabilities.dtype
        )
    ...
    if random.random() < 0.1:
        ...
        print(
            "-----------[avg_rewards]----------",
            sum(accumulate_rewards[-50:]) / len(accumulate_rewards[-50:]),
            "\n",
            flush=True,
        )
    return rewards
...
def get_reward_by_gpt(prompt):
    ...
    reward_bool = eval(response.choices[0].message.content)
...
あなたのタスクは、モデルの応答を参照回答と比較して評価することです。モデルの応答が正しいか、参照回答と一致している場合は「True」を出力します。不正確であるか、正しいオプションを選択できない場合は「False」を出力します。"""
...
あなたのタスクは、モデルの応答を参照回答と比較して評価することです。モデルの応答が正しいか、参照回答と一致している場合は「True」を出力します。不正確であるか、正しいオプションを選択できない場合は「False」を出力します。"""
    ...
    processed_batch = []
    output_matches = []
    rewards = torch.zeros(len(sub_answer), device=model.device, dtype=torch.bfloat16)
    for i in range(len(sub_answer)):
        response = tokenizer.decode(response_ids[i], skip_special_tokens=True)
        ...
        format_response = tmp.format(response, sub_answer[i])
        processed_batch.append(format_response)
        ...
        if output_match is None:
            ...
            reward_bool = get_reward_by_gpt(format_response)
            if reward_bool:
                rewards[i] = 1.0
            else:
                rewards[i] = 0.1
        ...
        with unwrap_model_for_generation(
            ...
            for batch in self.eval_dataloader:
                ...
                with torch.no_grad():
                    ...
                    table["answer"].extend(gather_object(allanswer))
